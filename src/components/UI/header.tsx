...
export default function Header() {
    function renderHeaderData() {
        ...
        return localConfigs.headerContnet.map((item) => {
            ...
            return (
                <div className={`${isActive ? "border-b-2 border-[#fd993a]" : 'border-none'}`} key={item.href}>
                    <Link href={item.href}>
                        {item.contnet}
                    </Link>
                </div>
        ...
        <div>
            <div className="flex py-2">
                ...
                <div className="relative my-5 ml-5">
                    <input placeholder="Поиск" className="p-2 w-52 pl-10 rounded-4xl border-[#10162333] border placeholder:text-gray-400"/>
                </div>
            </div>
        </div>
    )
